...
class RAGTravelAssistantService:
    def answer_query(self, db: Session, req: AIAssistantQueryRequest) -> AIAssistantResponse:
        ...
        answer_text = self._generate_grounded_answer(req.query, loc_outputs, context_snippets, req.budget_max, req.currency)
    ...
    def _generate_grounded_answer(
        ...
        """Creates grounded advice citing real reviews."""
        if not locations:
        ...
        # If Gemini API is available, generate dynamic grounded text
        if settings.GEMINI_API_KEY:
            try:
                ...
                prompt = (
                    ...
                    f"User Query: {query}\n"
                    f"Max Budget: {currency or '$'} {budget_max if budget_max else 'Any'}\n\n"
                ...
                logger.warning(f"LLM Generation for RAG query failed: {e}")
